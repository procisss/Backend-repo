...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (invId) {
        const existing = await db.execute(`SELECT id, stock_price FROM ingredient WHERE id = ${invId} AND user_id = ${req.user.id}`);
        if (existing.rows.length) {
          const oldPrice = existing.rows[0].stock_price || 0;
          await db.execute({ sql: `UPDATE ingredient SET quantity = quantity + ?, stock_price = ?, updated_at = datetime('now') WHERE id = ? AND user_id = ?`, args: [qty, +(oldPrice + cost).toFixed(2), invId, req.user.id] });
        }
      } else {
        const nameMatch = await db.execute(`SELECT id, stock_price FROM ingredient WHERE user_id = ${req.user.id} AND LOWER(TRIM(name)) = LOWER(TRIM('${nameClean.replace(/'/g, "''")}'))`);
        if (nameMatch.rows.length) {
          const existId  = nameMatch.rows[0].id;
          const oldPrice = nameMatch.rows[0].stock_price || 0;
          await db.execute({ sql: `UPDATE ingredient SET quantity = quantity + ?, stock_price = ?, updated_at = datetime('now') WHERE id = ?`, args: [qty, +(oldPrice + cost).toFixed(2), existId] });
        } else {
          await db.execute({ sql: `INSERT INTO ingredient (user_id, name, category, quantity, unit, stock_price, min_stock) VALUES (?, ?, 'Others', ?, ?, ?, 0)`, args: [req.user.id, nameClean, qty, unit, +cost.toFixed(2)] });
        }
      }
    }
